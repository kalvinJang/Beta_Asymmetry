...
def backtest(stock, market, screen):
    ...
    downbeta_ = downBeta.beta * screen
    ...
    pf_return, pf_value = backtesting_sketch(
                            ...
                            weight = myStrategy.total_weight.copy(),
                            resample = 'w-fri' # Weekly Rebalancing
                )
    mom_weight = momentum(
                    pf_value = pf_value,
                    lookback =13,
                    method ='hurdle_strict')
    # Plot Momentum Based Weight for Dynamic Optimization
    plt.figure(figsize=(24,12))
    plt.xlabel("Date", fontsize=20)
    plt.ylabel("Momentum Based Weight", fontsize=20)
    plt.plot(mom_weight['weight'], color='blue')
    plt.legend(['momentum-weight'])
    plt.show()

    # Adjusted resampled weights
    filled_weight = weight_fill(myStrategy.total_weight, mom_weight).fillna(0)
    ...
    # Blue-Original_strategy_PF, Red-Momentum_Based_Weight_Adjusted PF, Green-S&P500
    ...
    # Visualized the Periodic Performance of Real PF
    for start_year in [1960,1970,1980,1990,2000,2010]:
        end_year = str(start_year+10)
        start_year = str(start_year)
        if pf_return.loc[start_year: end_year].shape[0] == 0:
            continue
        print(start_year, "~", end_year)
        plt.figure(figsize=(24,12))
        plt.plot((pf_return+1)[start_year:end_year].cumprod(), color='blue')
        plt.plot((mom_pf_return+1)[start_year:end_year].cumprod(), color='red')
